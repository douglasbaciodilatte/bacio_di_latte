...
/*/{Protheus.doc} 311MODELS
    @type  Ponto de Entrada MVC BDTRASF
    @author Felipe Mayer
    @since 11/05/2021
/*/

User Function 311MODELS()
            ...
            ElseIf nOper == 4
                If NNS->NNS_STATUS != "2"
                    If Date()>NNS->NNS_DATA .And. Upper(SubsTr(UsrRetName(RetCodUsr()),0,2))=='LJ'
                        ...
                        lRet := .F.
                    ElseIf Val(SubsTr(Time(),1,2)) >= cHrTrav .And. Upper(SubsTr(UsrRetName(RetCodUsr()),0,2))=='LJ'
                        MsgAlert("Não é possível a alteração após as "+cValToChar(cHrTrav)+"h00<br>Horário atual: "+Time())
                        lRet := .F.
                    EndIf
                Else
                ...
                If lRet .And. NNS->NNS_XCLASP == '2'
                    cFilAnt := '0136'
                ElseIf lRet .And. NNS->NNS_XCLASP == '2'
                    cFilAnt := '0072'
                EndIf
            EndIf
...
Local oDlg1,oFont1,oFont2,oBtn1,oBtn2,oSay1,oGrp1,oCBox1
    ...
    oCBox1 := TComboBox():New(040,008,{|u|if(PCount()>0,cCombo:=u,cCombo)},{"1 - Secos","2 - Congelados","3 - Bacio Casa"},;
    ...
    oBtn2  := TButton():New(064,068,"Cancelar",oDlg1,{||lRet:=.F.,cFilAnt:=__cBkpFila,oDlg1:End()},037,012,,oFont1,,.T.,,"",,,,.F.)
