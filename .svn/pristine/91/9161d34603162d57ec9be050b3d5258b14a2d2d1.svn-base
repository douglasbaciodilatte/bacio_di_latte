...
//ConOut("Quantidade de Orcamento:",len(_oData:Orcamento ))   //quantos orcamentos para o conaut
begin sequence
...
PIN->PIN_SERIE	:= Iif(!EMPTY(Orcamento:SerieSat), "", Orcamento:Serie) //IIF(TRC->ZTQ_TPPDV == "1",SUBSTR(TRC->ZTQ_CHAVE,23,3), "")			
...
PIN->PIN_ACAO	:= "1"
PIN->PIN_STAIMP	:= ""
PIN->PIN_RECCAB := PIN->(Recno()) //Orcamento:RECCAB
...
MsUnlock()
//ConOut("CUPOM...xCodexs PIN -"+_cXCodeX+" Recno:"+ Str(_nXRecno))
return _nXRecno
...
PIO->PIO_VEND	:= "000001"
PIO->PIO_SITUA	:= "RX"
PIO->PIO_SITTRI	:= "T0300" //"T"+STRZERO(_oItems[x]:AliqICM,2)+"00"
...
PIO->PIO_CODDES := '1'
PIO->PIO_ACAO	:= "1"
//PIO->PIO_PICM := _oItems[x]:AliqICM
MsUnLock()
//ConOut("ITEM...xCodexs-"+_cXCodeX+" - Reccab:"+ Str(_nRecPIO))
next
return .t.
