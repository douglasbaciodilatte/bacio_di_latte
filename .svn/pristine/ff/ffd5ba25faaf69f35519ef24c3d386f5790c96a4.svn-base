...
User Function BDLESTA2()
    ...
    aAdd(aHeadSBM, {"Armazém"           , "XX_LOCAL"  ,""                 ,TamSX3("D3_LOCAL")[01],0,"U_VlSE3Cpo('LOCAL')"                               , "û", "C", "NNR", "" } )
    ...
    aAdd(aHeadSBM, {"Total"             , "XX_CUSTO"  ,"@E 999,999.99"    ,010                  , 2,"U_VlSE3Cpo('CUSTO')"               , "û", "N", ""   , "" } )

    aAlter := {"XX_COD","XX_LOCAL","XX_TM","XX_QUANT","XX_QUANT2","XX_EMISSAO"}    
	
    aAdd(aColsSBM, { ;
            SPACE(TamSX3("D3_COD")[01] ),;//SPACE(TamSX3("B1_LOCPAD")[01]),;
            SPACE(TamSX3("B1_DESC")[01]),;
            SPACE(TamSX3("B1_LOCPAD")[01]),;
            SPACE(3),;
            SPACE(2),;
            0,;
            SPACE(2),;
...
user function VLSE3()
    Local aColsAux    := oMsGetSBM:aCols
    Local nPosCod     := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_COD"})
    Local nPosLocal   := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_LOCAL"})
    Local nPosTM      := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_TM"})
    Local nPosQuant   := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_QUANT"})
    Local nPosEmissao := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_EMISSAO"})
    ...
    elseif !(ExistCpo("NNR", aColsAux[nLinha,nPosLocal]))
        lRet := .F.
...
USER Function VlSE3Cpo(cTipo)
    ...
    Local nPosUM1     := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_UM"     })
    Local nPosUM2     := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_UM2"    })
    Local nPosCust    := aScan(aHeadSBM, {|x| Alltrim(x[2]) == "XX_CUSTO"  })
    Local cLocal      := ""
    ...
    if M->XX_QUANT <> NIL
        oMsGetSBM:aCols[nX,nPosQuant] := M->XX_QUANT
    else
        M->XX_QUANT := oMsGetSBM:aCols[nX,nPosQuant] 
    endif
/*
    if M->XX_LOCAL <> NIL
        oMsGetSBM:aCols[nX,nPosLocal] := M->XX_LOCAL
    else
        M->XX_LOCAL := oMsGetSBM:aCols[nX,nPosLocal] 
    endif
    */
    ...
    if cTipo == "LOCAL" .and. M->XX_LOCAL <> NIL//!Empty(oMsGetSBM:Acols[nX,nPosLocal])
        cLocal:= posicione("NNR",1,XfILIAL("NNR")+ M->XX_LOCAL,"NNR_DESCRI" )
        IF Empty(cLocal)
            lRet := .f.
            Alert("Armazém Invalido!!! Verifique")
        EndIf
    EndIf
    ...
    nCusto := posicione("SB1",1,XfILIAL("SB1")+oMsGetSBM:aCols[nX,nPosCod],"B1_CUSTD")
    M->XX_CUSTO := nCusto * M->XX_QUANT //oMsGetSBM:aCols[nX,nPosQuant] 
    oMsGetSBM:aCols[nX,nPosCust] := M->XX_CUSTO
...
Static Function xArray( )
    ...
    //Local nPosLocal := aScan(aCabAux, {|x| Alltrim(x[2]) == "XX_LOCAL"  })
    //Local nPosCust  := aScan(aCabAux, {|x| Alltrim(x[2]) == "XX_CUSTO"  })
    ...
    For nI := 1 To Len( aCabAux)
       ...
       // elseIf nI == nPosCust .or. nI == nPosLocal
            //não faz nada
        else
...
Static Function xDefArray( aCOLS, aHeader )
    ...
    */
    ...
    //For nX := 1 To nLen
        ...
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+01)) OF oSection SIZE 045 TITLE aHeader[02,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+02)) OF oSection SIZE 006 TITLE aHeader[03,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+03)) OF oSection SIZE 032 TITLE aHeader[04,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+04)) OF oSection SIZE 004 TITLE aHeader[05,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+05)) OF oSection SIZE 014 TITLE aHeader[06,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+06)) OF oSection SIZE 004 TITLE aHeader[07,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+07)) OF oSection SIZE 014 TITLE aHeader[08,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+08)) OF oSection SIZE 010 TITLE aHeader[09,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+09)) OF oSection SIZE 014 TITLE aHeader[10,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+10)) OF oSection SIZE 014 TITLE aHeader[11,1]
        DEFINE CELL NAME "CEL"+Alltrim(Str(nX+11)) OF oSection SIZE 014 TITLE aHeader[12,1]
    //Next nX
        DEFINE CELL NAME "LINHA01" OF oSection2 SIZE 015 TITLE " "
        DEFINE CELL NAME "LINHA02" OF oSection2 SIZE 045 TITLE " "
        DEFINE CELL NAME "LINHA03" OF oSection2 SIZE 006 TITLE " "
        DEFINE CELL NAME "LINHA04" OF oSection2 SIZE 032 TITLE " "
        DEFINE CELL NAME "LINHA05" OF oSection2 SIZE 004 TITLE " "
        DEFINE CELL NAME "LINHA06" OF oSection2 SIZE 014 TITLE " "
        DEFINE CELL NAME "LINHA07" OF oSection2 SIZE 004 TITLE " "
        DEFINE CELL NAME "LINHA08" OF oSection2 SIZE 014 TITLE " "
        DEFINE CELL NAME "LINHA09" OF oSection2 SIZE 014 TITLE " "
        DEFINE CELL NAME "LINHA10" OF oSection2 SIZE 014 TITLE " "
        DEFINE CELL NAME "TOTAL"   OF oSection2 SIZE 014 TITLE " "
    /*
    +---------------------------------------+    
    ...
    */
    ...
    //oSection:aCell[03]:disable()
    oSection:aCell[10]:disable()
...
Static Function xImprArray( oReport, aCOLS )
    ...
    Local oSection2 := oReport:Section(2) // Retorna objeto da classe TRSection (seção). Tipo Caracter: Título da seção. Tipo Numérico: Índice da seção segundo a ordem de criação dos componentes TRSection.
    Local nX := 0
    Local nY := 0
    Local nTotal := 0
    ...
    */
    ...
    oSection2:Cell("LINHA10"):SetBlock( &("{ || 'Total:'}") )
    oSection2:Cell("TOTAL"):SetBlock( &("{ || '" + TransForm(nTotal,'@E 99,999,999.99') + "'}") )
    /*
...
Static function fquery()
    ...
    cQuery +=  "        ROUND((D3_QUANT * B1_CUSTD),2) AS XX_CUSTO "
...
static Function xCab()
    ...
    aAdd(aCab, {"Armazém"           , "XX_LOCAL"  , ""                ,6                    , 0})
...
Static function xAcols()
    ...
    While TRB1->(!EOF())
        ...
        AAdd(aCols,{TRB1->XX_COD,TRB1->XX_DESC,TRB1->XX_LOCAL,TRB1->XX_TM,TRB1->XX_UM,TRB1->XX_QUANT,TRB1->XX_UM2,TRB1->XX_QUANT2,Stod(TRB1->XX_EMISSAO), TRB1->XX_CUSTO,.F.})
...
*/
/*
1 Unidade x B1_CUSTD campo valor
2 Armzém na linha selecionar
3 Relatorio contendo as TM e emissão adicionar no menu

*/

User Function RELESTA2()

Local aCombo := {"Emissão","Produto"}
Local aParam := {{1,"Emissão de" ,Ctod(Space(8)),,"","",".T.",80,.F.},;
                 {1,"Emissão Até",Ctod(Space(8)),,"","",".T.",80,.T.},;
                 {2,"Ordenar por",2,aCombo,50,"",.T.},;
                 {1,"Armazém",Space(TamSx3('D3_LOCAL')[1]),"","","NNR","",50,.F.} }

    If ParamBox(aParam,"Parametros",,,,.F.,420,5) 
        Processa({|| fGeraExcel()},"RELESTA2","Processando, aguarde...",.F.)
    EndIf
...
Local cQuery := ''
    ...
    cQuery := CRLF + " SELECT D3_FILIAL  AS XX_FILIAL , "
    cQuery += CRLF + "        D3_COD     AS XX_COD , " 
    cQuery += CRLF + "        B1_DESC    AS XX_DESC , "
    cQuery += CRLF + "        D3_LOCAL   AS XX_LOCAL , "
    ...
    cQuery += CRLF + "        D3_EMISSAO AS XX_EMISSAO , "
    cQuery += CRLF + "        ROUND((D3_QUANT * B1_CUSTD),2) AS XX_CUSTO "
    ...
    cQuery += CRLF + "            AND B1_FILIAL     = '' "
    cQuery += CRLF + " WHERE   D3_TM IN('510','520','530','540','550') " 
    cQuery += CRLF + "        AND D3_CF = 'RE0' "
    cQuery += CRLF + "        AND D3_FILIAL NOT IN ('0031','0072') "
    cQuery += CRLF + "        AND D3.D_E_L_E_T_ = '' "
    cQuery += CRLF + "        AND D3_EMISSAO BETWEEN '"+DToS(MV_PAR01)+"' AND '"+DToS(MV_PAR02)+"' "

    If !Empty(MV_PAR04)
        cQuery += CRLF + " AND D3_LOCAL = '"+MV_PAR04+"' "
    EndIf

    If MV_PAR03 == 1
        cQuery += CRLF + "    ORDER BY D3_FILIAL,D3_EMISSAO,D3_COD"
    Else
        cQuery += CRLF + "    ORDER BY D3_FILIAL,D3_COD,D3_EMISSAO"
    EndIf

    U_QRYCSV(cQuery,"REL_DESCARTES")
