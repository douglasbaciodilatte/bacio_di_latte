...
User Function BLESTR31()
	...
	//aAdd(aParamBox,{1,"Até Data"  ,Ctod(Space(8)),"","","","",50,.T.})
    ...
    IF Pergunte(cPerg,.T.)
        ...
        fQCONSTo()
        Processa( {|| fGeraExel(cPasta)}, "Gerando Planilha Consumo Teórico...")
    EndIf    
...
Default nSlv    := 1
    ...
    while !TCOT->(EOF())
        ...
        cTabela   := "Consumo Teórico: " + cLjFil 
...
//Define a Tabela na Planilha
    ...
    oExcel:AddColumn(cPlanilha , cTabela , "Descrição"              , 1 , 1 , .f. ) //C
    oExcel:AddColumn(cPlanilha , cTabela , "Produto Comp"           , 1 , 1 , .f. ) //D
    oExcel:AddColumn(cPlanilha , cTabela , "Descrição"              , 1 , 1 , .f. ) //E
