	...
	For nCountNF := 1 To Len(aCupRefLoj)
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_QUANT, D2_QTSEGUM,D2_SEGUM, D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
								D2_CLASFIS,D2_PRCVEN,D2_SEGUM, D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If SC6->(FieldPos("C6_NUMPCOM")) > 0 .And. SC6->(FieldPos("C6_ITEMPC")) > 0
							...
							Else	
								//Bacio Di Latte: Alterado por Douglas Silva 05/11/2020 / Numero pediod do Cliente/Neogrid
								If SC5->(FieldPos("C5_XPEDCLI")) > 0 
									If ! EMPTY( SC5->C5_XPEDCLI )						
										aadd(aPedCom,{ SC5->C5_XPEDCLI , SC6->C6_ITEM })																
									Else
										aadd(aPedCom,{})
									Endif
								EndIf	
						...
						cCodProd  := (cAliasSD2)->D2_COD	            
             			s_Segum	  := (cAliasSD2)->D2_SEGUM 

						s_DSegum  := POSICIONE('SAH',1,xFilial("SAH")+s_Segum,"AH_UMRES")    
           			                                                                                    
						s_QtSegum := ConvUm(cCodProd, (cAliasSD2)->D2_QUANT ,0,2)  //Alterado por Douglas Silva 05/10/2020
						...
						//BDIL: DOUGLAS SILVA 29/09/2020 - Inclusão lote para notas fiscais de saída								            
			            If lCpoMsgLT .And. SF4->F4_MSGLT $ "1" 
							
							aAreaSD2  	:= SD2->(GetArea())
							dbSelectArea("SD2")
							dbSetOrder(3) //D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM

							If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD +(cAliasSD2)->D2_ITEM )

								cNumLotForn := SD2->D2_LOTECTL
								If ! Empty(cNumLotForn)
									cInfAdic := "LOTE " + Alltrim(cNumLotForn) + " VALIDADE " + DTOC(SD2->D2_DTVALID) + " " + cInfAdic
								EndIf	
