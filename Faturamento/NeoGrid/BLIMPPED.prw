...
For nX := 1 to Len(aFiles)
	...
	If DbSeek(xFilial("SC5")+cVNum) //Criar um Indice para o Pedido do Cliente para que ao veriricar se o pedido informado no arquivo já exista será uma alteração ou exclusão.
		...
		MsgAlert("ATENÇÃO: Pedido Cliente: " + cVNum  + " Pedido Protheus: " + SC5->C5_NUM + " Já existe na base!"  )
		...
		If subs(cbuffer,0,02) == "04"
			...
			While lItem
				...
				*/
				...
				Else
					xPrcTab:=0
					cErroPro +="Nao existe preço de vendas cadastrado na Tabela de precos para o produto " + xPRODUTO + "  o valor que sera assumido sera o que consta no arquivo enviado pelo cliente caso o problema nao seja o cadastro do produto, por favor verificar" + CHR(13) + CHR(10)
...
U_FTPPED(.T.)
 ...
 //Verifica se usuário está logado filial 0072

if cEmpAnt == "0072" 
	Alert("ATENÇÃO: Verifique a filial logada, rotina habilitada somente para filial 0072")
	Return Nil
endif	
