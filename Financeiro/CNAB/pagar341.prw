...
Do Case
	...
	Case _cTipo == "PP000"	//  Código do Banco Favorecido
	_cBco := " "
	IF EMPTY(SE2->E2_FORBCO)
			_cBco := strzero(val(SA2->A2_BANCO),3,0)
		ELSE
			_cBco := strzero(val(SE2->E2_FORBCO),3,0)  
	EndIf
	_cRetorno:= _cBco
	...
	Case _cTipo == "PP001"	//  Agencia e Conta Corrente Favorecido
		...
		IF EMPTY(SE2->E2_FORBCO)
			_cConta := strzero(val(SA2->A2_NUMCON),10,0)
		ELSE
			_cConta := strzero(val(SE2->E2_FORCTA),10,0)
		EndIf
		...
		If SE2->E2_FORBCO == "341"
				_cRetorno := "0"+strzero(val(substr(se2->e2_forage,1,4)),4)+" "+"0000000"
				_cRetorno += strzero(val(substr(_cConta,6,5)),5)+" "+substr(se2->e2_fctadv,1,1)
			Else
				_cRetorno := strzero(val(substr(se2->e2_forage,1,5)),5)+" "
				_cRetorno += strzero(val(substr(_cConta,1,12)),12)
				If len(alltrim(se2->e2_fctadv)) > 1
					_cRetorno += strzero(val(substr(se2->e2_fctadv,1,2)),2)
				Else
					_cRetorno += " "+substr(se2->e2_fctadv,1,1)
				EndIf
			EndIf
		EndIf
		//--- Mensagem ALERTA
		/*
		If SEA->EA_MODELO <> '10' .AND. (Empty(SA2->A2_AGENCIA) .or. Empty(_cConta))
		...
		*/
		...
		If Empty(se2->e2_forage) .or. Empty(_cConta)
	...
	Case _cTipo == "PT000"
		...
		ElseIf  SEA->EA_MODELO == "18"     // Posicao 018 a 019: Identificacao do Tributo  03-Darf Simples
			_cRetorno := "03"
		ElseIf SEA->EA_MODELO == "22" .OR. SEA->EA_MODELO == "21"  // Posicao 018 a 019: Identificacao do Tributo  05-ICMS //DARJ
			_cRetorno := "05"
	...
	Case _cTipo == "PT001"
		...
		//  Dados DARF SEM CODIGO DE BARRAS ( deve conter codigo recolhimento , periodo de apuração, SEGMENTO N)
		If SEA->EA_MODELO == "16"
...
//			_cRetorno += Strzero(SE2->E2_XVLENT*100,14)
			...
			_cRetorno += Strzero(SE2->E2_ACRESC+SE2->E2_JUROS*100,14)
			...
			//----- DARF SIMPLES / CONTEM CODIGO DE BARRAS (SEGMENTO O)
		ElseIf  SEA->EA_MODELO == "18"
			...
			//--- GARE ICMS SP
		ElseIf SEA->EA_MODELO == "22" .OR. SEA->EA_MODELO = "21" //--- GARE ICMS - SP
			...
			_cRetorno +=  Subs(SM0->M0_CGC,1,14) //--- Funcao u_dadosSM0 está no fonte PAGAR356.PRW
			
			//--- Posicao 039 a 050: Inscricao Estadual-

			_cRetorno +=  Subs(SM0->M0_INSC,1,12)   //--- Funcao u_dadosSM0 está no fonte PAGAR356.PRW			
		...
		ElseIf SEA->EA_MODELO == "25"  .or. SEA->EA_MODELO == "26" .or. SEA->EA_MODELO == "27"
			...
			_cRetorno +=  Strzero(Val(SE2->E2_XRENAV),9)
			
			//--- Posicao 052 a 053 - Unidade Federação
			_cRetorno +=  Upper(SE2->E2_XIPVUF)
			
			//--- Posicao 054 a 058 - Codigo do Municipio
			_cRetorno += Strzero(Val(SE2->E2_XCODMUN),5)
			
			//--- Posicao 059 a 065 - Placa
			_cRetorno +=  SE2->E2_XPLACA
			
			//--- Posicao 066 a 066 - Opção de Pagamento - Para DPVAT sempre opção 0
			If SEA->EA_MODELO == "25"
				_cRetorno += Alltrim(SE2->E2_XOPCAO)
			Else
		...
		ElseIf SEA->EA_MODELO == "35"
			...
			_cRetorno += Strzero(Val(SE2->E2_XIDENT),16)
			
			// Posicao 103 a 111: Lacre de Conectividade Social
			_cRetorno += Strzero(Val(SE2->E2_XLACRE),9)
			
			// Posicao 112 a 113: Digito do Lacre de Conectividade Social
			_cRetorno += Strzero(Val(SE2->E2_XDGLACR),2)
			...
			_cRetorno += STRZERO((SE2->E2_SALDO+SE2->E2_ACRESC+SE2->E2_JUROS-SE2->E2_DECRESC)*100,14)
...
User Function PAGAR342()
cRetbcag:=""
cRetbcag:=IF(SEA->EA_MODELO="10","00845",STRZERO(VAL(SA2->A2_AGENCIA),5))    
Return(cRetbcag)
